...
This module implements the baseline DyMixOp neural operator for learning
spatiotemporal dynamics in 1D, 2D, and 3D domains.

Architecture:
    Input → Lift → Reduction → DyIA(LGM Layers) → Inverse_reduction → Inverse_lifting → Output

Key Components:
    - GlobalTransformation: Fourier spectral convolution for global patterns
    - LocalTransformation: Convolution/Finite difference for local patterns
    - LGMTransformation: Hadamard product of global and local (mixing)
    - LGMLayer: Combined linear and nonlinear dynamics
    - DyIA: Hybrid sequential-parallel layer connectivity
"""
...
class LocalTransformation(nn.Module):
    ...
    def forward(self, c: torch.Tensor, grid_width: float) -> torch.Tensor:
...
class LGMTransformation(nn.Module):
    """Local-Global mixing (Hadamard product of GT and LT)."""
    
    def __init__(self, in_dim: int, out_dim: int, device, modes: int, ks: int, 
                 coor_dim: int, approx_nonlin: bool = True):
        super().__init__()
...
class LGMLayer(nn.Module):
    """LGM layer with nonlinear and linear branches."""
    
    def __init__(self, in_dim: int, out_dim: int, device, num_nonlin: int, num_lin: int,
                 A_ks: int, coor_dim: int, modes: int, ks: int):
        super().__init__()
        ...
        for _ in range(num_nonlin):
            self.nonlin_layers.append(LGMTransformation(in_dim, out_dim, device, modes, ks, coor_dim, True))
        
        # Linear branches (conv + diff)
        ...
        # Residual operator A: Conv(c→2c→c)
    ...
    def forward(self, c: torch.Tensor, grid_width: float) -> torch.Tensor:
        ...
        lin_sum = sum(layer(c, grid_width) for layer in self.lin_layers)
        return lin_sum + self.A_op(nonlin_sum)


# ==============================================================================
# Dynamics Informed Architecture
# ==============================================================================

class DyIA(nn.Module):
    """DyIA: Hybrid sequential-parallel architecture with adaptive gating."""
    ...
    def __init__(self, in_dim: int, out_dim: int, device, num_layers: int, num_nonlin: int,
        ...
        self.layers = nn.ModuleList([
            LGMLayer(in_dim, out_dim, device, num_nonlin, num_lin, A_ks, coor_dim, modes, ks)
    ...
    def forward(self, c: torch.Tensor, grid_width: float = 1, temp_coeff: torch.Tensor = None) -> torch.Tensor:
        c_init = c
        outputs = []
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Sequential update (except last layer)
            if i < self.num_layers - 1:
                eff = self.lambda_ * temp_coeff if temp_coeff is not None else self.lambda_
                c = c + F_c * torch.sigmoid(eff)
            
            outputs.append(F_c)
        
        # Parallel weighted sum
        stacked = torch.stack(outputs, dim=2)
        view = stacked.view(-1, self.num_layers, *c_init.shape[2:])
        return c_init + self.evo_weights(view).view(c_init.shape[0], -1, *c_init.shape[2:])
...
class DyMixOp(nn.Module):
    """DyMixOp for 1D, 2D, and 3D spatiotemporal data."""
    ...
    def __init__(self, config, device: torch.device):
        ...
        self.infer_factor = getattr(config, 'infer_scaling_factor', 1)
        
        self.lift = make_conv(conv_class, self.input_ch, v_dim, ks, device)
        self.reduction = make_conv(conv_class, v_dim, c_dim, ks, device)
        ...
        self.inverse_reduction = make_conv(conv_class, c_dim, v_dim, ks, device)
        self.inverse_lifting = make_conv(conv_class, v_dim, config.output_dim, ks, device)
    ...
    def _step(self, u: torch.Tensor, temp_coeff, grid_width: float = 1) -> torch.Tensor:
        v = self.lift(u)
        c = self.reduction(v)
        c = self.dyia(c, temp_coeff, grid_width)
        v = self.inverse_reduction(c)
        return self.inverse_lifting(v)